...
trimFasta <- function(fa,min_sites_per_seq,min_seqs_per_site) {
	...
	iter <- iter+1
	cat("\nTrimming iteration\t",iter,"\n")
		# Find and remove gappy sites
		if (min_seqs_per_site > 1){
			is_empty_site <- !apply(do.call(rbind,fa),2,meetsNumNonGap,num_nonGap=min_seqs_per_site)
		} else {
			is_empty_site <- !apply(do.call(rbind,fa),2,meetsPropNonGap,prop_nonGap=min_seqs_per_site)
		}
		# print(which(is_empty_site))
		if (any(is_empty_site)) {
			cat(sum(is_empty_site),"out of",fa_width,"SITES have <",min_seqs_per_site,"	sequences with data (not N, not gap). Removing them.\n")
			fa <- lapply(fa,function(x) x[-which(is_empty_site)])
			found_missing_sites<-TRUE
		} else {
			cat("No (effectively) missing sites.\n")
			found_missing_sites<-FALSE
		}
		# Find and remove gappy sequences
		if (min_sites_per_seq > 1) {
			is_empty_seq <- !unlist(lapply(fa,meetsNumNonGap,num_nonGap=min_sites_per_seq))
		} else {
			is_empty_seq <- !unlist(lapply(fa,meetsPropNonGap,prop_nonGap=min_sites_per_seq))
		}
		if (any(is_empty_seq)) {
			cat(sum(is_empty_seq),"out of",length(fa),"SEQUENCES have <",min_sites_per_seq,"sites with data (not N, not gap). Removing them.\n")
			cat("Sequences removed:",names(fa)[is_empty_seq],sep = "\n")
			found_missing_seqs<-TRUE
			fa <- fa[!is_empty_seq]
		} else {
			cat("No (effectively) empty sequences.\n")
...
## Begin work! ----
if (trim_before==1) {
	cat("\nPerforming initial fasta filtering to remove sequences with <",MinSitesPerSeq,"non-gap/non-N sites.\n")
	cat("Note that no sites (except sites with < 3 sequences) will be removed in this round!\n")
	gene <- trimFasta(gene, min_sites_per_seq = MinSitesPerSeq, min_seqs_per_site = 3)
} else {
	## just trim to remove all-gap sequences and sites
	cat("\nYou have opted out of initial fasta filtering. Only empty sequences and sites will be trimmed.\n")
	gene <- trimFasta(gene, min_sites_per_seq = 0, min_seqs_per_site = 0)
}
...
if (trim_after==1) {
	cat("\nPost-chomp trimming.\n\n")
	gene <- trimFasta(gene, min_sites_per_seq = MinSitesPerSeq, min_seqs_per_site = MinSeqsPerSite)
} else {
	## just trim to remove all-gap sequences and sites
